...
class keras_model:
    ...
    def __init__(self, df=None, text=None, label=None, test_split=0.10, lr=0.01,
                 ...
                 test_df=None, cnn=False, act='softmax', lstm=True, opt='adam', drop=False, slim_bert=False, attention=True):
        ...
        if isinstance(df, pd.DataFrame) and text!=None and label!=None:
            ...
            self.slim_bert = slim_bert

            self.save_em_dict = pre_trained_EM
    ...
    def save_model(self, name):
        if self.slim_bert:
            ...
            model_infor_list = [self.model.get_weights(), self.save_em_dict, self.df, self.text, self.label]
            #name = name + '_weights.h5'
            #self.model.save_weights(name)
            name = name + '.pkl'
            with open(name, 'wb') as output:  # Overwrites any existing file.
                pickle.dump(model_infor_list, output, pickle.HIGHEST_PROTOCOL)
        else:
            name = name + '.pkl'
            with open(name, 'wb') as output:  # Overwrites any existing file.
        ...
        if self.slim_bert:
            warnings.warn('please use restore_attention_model() to load attention/slim_bert model')
...
#load a keras_model or any pkl object
def load_pkl(name):
    #warnings.warn("if you are loading a keras_model\nkeras does not able to recognize custom layers")
    warnings.warn('use restore_attention_model() to load a attention/slim_bert model')
    print("---------------------")
    print("loading")
    try:
...
# a dumb way to load keras_model with custom attention layer
# when use load_pkl, keras does not able to load "SeqSelfAttention"
def restore_attention_model(path):
    l = load_pkl(path)
    try:
        ret_model = keras_model(l[2], l[3], l[4], slim_bert=True, lstm=False, embedding_dim=len(l[0][0][0]), pre_trained_EM=l[1])
        #ret_model = keras_model(l[1], l[2], l[3], slim_bert=True, lstm=False)
        ret_model.model.set_weights(l[0])
    except:
        try:
            ret_model = keras_model(l[2], l[3], l[4], slim_bert=True, embedding_dim=len(l[0][0][0]), pre_trained_EM=l[1])
            #ret_model = keras_model(l[1], l[2], l[3], slim_bert=True)
            ret_model.model.set_weights(l[0])
        except:
            ret_model = keras_model(l[2], l[3], l[4], slim_bert=True, drop=True, embedding_dim=len(l[0][0][0]), pre_trained_EM=l[1])
            #ret_model = keras_model(l[1], l[2], l[3], slim_bert=True, drop=True)
            ret_model.model.set_weights(l[0])
